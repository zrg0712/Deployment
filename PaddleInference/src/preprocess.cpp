...
{
	void InitInfo::Run(cv::Mat* im, ImageBlob* data)  // 获取Imageblob信息
	{
	...
	{
		double e = 1.0;
		if (is_scale_) e /= 255.0;

		(*im).convertTo(*im, CV_32FC3, 3);

		if (norm_type_ == "mean_std")
		{
			for (int h = 0; h < im->rows; h++)  // 遍历每个像素点
			{
				for (int w = 0; w < im->cols; w++)
				{
					im->at<cv::Vec3f>(h, w)[0] = (im->at<cv::Vec3f>(h, w)[0] - mean_[0]) / scale_[0];
					im->at<cv::Vec3f>(h, w)[1] = (im->at<cv::Vec3f>(h, w)[1] - mean_[1]) / scale_[1];
					im->at<cv::Vec3f>(h, w)[2] = (im->at<cv::Vec3f>(h, w)[2] - mean_[2]) / scale_[2];
				}
			}
		}
	}
	...
	void Permute::Run(cv::Mat* im, ImageBlob* data)
	{
		...
		int rw = im->cols;
		int rc = im->channels();
		(data->im_data_).resize(rc * rh * rw); // 调整大小，以容纳数据
		float* base = (data->im_data_).data();  // 获取了调整大小后向量的数据指针base。该指针将用于填充来自每个通道的数据。
		for (int i = 0; i < rc; ++i) {  // 从输入图像中提取每个通道的数据，并将其存储到im_data_向量
			/*cv::extractChannel用于从图像(*im)中提取单个通道。
			提取的通道数据被放入一个指定大小(rh, rw)和数据指针(base + i * rh * rw)的cv::Mat中。
			循环遍历每个通道。*/
	...
	void Resize::Run(cv::Mat* im, ImageBlob* data)
	{
		...
		cv::resize(*im, *im, cv::Size(), resize_scale.first, resize_scale.second, interp_);  // interp_：插值方式
		
		data->in_net_shape_ = { static_cast<float>(im->rows),
							   static_cast<float>(im->cols) }; // 传入模型中的尺寸
		data->im_shape_ = {
			static_cast<float>(im->rows), static_cast<float>(im->cols),
		};
		data->scale_factor_ = {  // 保存图像缩放参数，方便后处理
