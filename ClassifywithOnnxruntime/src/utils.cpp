...
void utils::visualizeDetection(cv::Mat& image, std::vector<Result>& detections, const std::vector<std::string>& classNames)
{
    for (const auto& res : detections)
    {
        cv::rectangle(image, res.box, cv::Scalar(229, 150, 20), 2);

        int x = res.box.x;
        int y = res.box.y;

        int conf = (int)std::round(res.conf * 100);
